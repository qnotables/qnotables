...
import { saveAdminFriend, saveFriend, type FriendFormState } from "@/app/dashboard/friends/actions"
    ...
    <form action={action} className="flex flex-col gap-6">
      ...
      <div className="border border-border bg-card p-5">
        <div className="mb-5 flex items-end justify-between gap-4 border-b border-border pb-4">
          <div><p className="label-mono text-primary">{adminMode ? 'ADMIN EDITOR' : 'FRIEND LISTING'}</p><h1 className="stencil mt-1 text-2xl">{friend ? 'Edit Friend' : 'Add a Friend'}</h1></div>
          <span className="label-mono text-xs text-muted-foreground">{adminMode ? `CURRENT STATUS: ${friend?.status?.replace('_', ' ') ?? 'unknown'}` : 'PUBLIC AFTER APPROVAL'}</span>
        </div>
