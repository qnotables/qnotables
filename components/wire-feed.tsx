...
export function WireFeed({ desks, isLoggedIn }: { desks: Desk[]; isLoggedIn: boolean }) {
  ...
  return (
    <details id="wire" className="group mt-12 scroll-mt-40 border-y border-border">
      <summary className="flex cursor-pointer list-none items-center gap-3 py-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background [&::-webkit-details-marker]:hidden">
        ...
        <span className="label-mono text-xs text-muted-foreground group-open:hidden">Show wire</span>
        <span className="label-mono hidden text-xs text-muted-foreground group-open:inline">Hide wire</span>
        <span
          aria-hidden="true"
