...
function ActivityPanel({ profile, activeTab, activity }: { profile: MockProfile; activeTab: ProfileTab; activity: MockProfile["posts"] | MockProfile["replies"] }) {
  return <div className="flex flex-col gap-4 pt-5">{activity.length === 0 ? <EmptyProfileState title={`No ${activeTab} yet`} body="Public contributions will appear here when this member adds them." /> : activity.map((item) => <Card key={`${item.title}-${item.date}`} className="transition-colors hover:border-primary/50"><CardHeader><div className="flex items-center justify-between gap-3"><Badge variant="outline">{item.topic}</Badge><span className="text-xs text-muted-foreground">{item.date}</span></div><CardTitle className="font-serif text-xl leading-tight">{item.title}</CardTitle><Markdown content={item.excerpt} className="gap-2 leading-6 [&_p]:leading-6" /></CardHeader><CardContent className="pt-0 text-xs text-muted-foreground">{"replies" in item ? `${item.replies} replies` : <span className="inline-flex items-center gap-1"><MessageCircle className="size-3.5" /> Reply in a discussion</span>}</CardContent></Card>)}</div>
}
