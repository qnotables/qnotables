...
export function EditProfileForm({ initialProfile }: { initialProfile?: MockProfile }) {
  ...
  const save = async () => {
    ...
    const response = await fetch("/api/profile/update", {
      ...
      body: JSON.stringify({
        ...
        featuredThreadIds: profile.featured?.filter((item) => item.type === "thread").map((item) => item.id) ?? [],
      }),
    })
    if (!response.ok) {
  ...
  return <div className="min-h-screen bg-background"><main className="mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-12">
    ...
    {saved && <div role="status" className="mb-6 flex items-center gap-3 border border-primary/30 bg-primary/10 px-4 py-3 text-sm text-foreground"><Check className="size-4 text-primary" /> Your profile changes are saved and visible on your public profile.</div>}
