    ...
    >
      {src ? (
        <Image
          ...
          alt={alt}
          fill
          className="object-contain object-top opacity-90 transition-opacity duration-300"
...
function BlogHotCard({ item }: { item: SituationBlogItem }) {
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      {/* First body media is promoted without changing the remaining content order. */}
      {media ? (
        <div className="relative border-b border-border">
          <PostFeaturedMedia media={media} title={item.title} compact />
          {item.featured && (
