      ...
      <div className="overflow-x-auto border border-border">
        <table className="w-full min-w-[980px] text-sm">
          ...
          <tbody>{changes.map((item) => <tr key={item.id} className="border-b border-border align-top">
            ...
            <td className="max-w-md px-4 py-3"><p className="font-semibold text-foreground">{item.title}</p><p className="mt-1 text-xs text-muted-foreground">{item.publisher ?? "Unknown publisher"} · {item.rationale}</p></td>
