          ...
          <div className="flex flex-col gap-4">
            {[...footerLinks, ...legalLinks].sort((a, b) => a.sortOrder - b.sortOrder).map((link, index, list) => (
              <article key={link.id} className="flex flex-col gap-4 border border-border bg-card p-4">
                ...
                <div className="grid gap-4 md:grid-cols-2"><Field label="Label"><Input value={link.label} maxLength={80} onChange={(event) => updateLink(link.id, { label: event.target.value })} /></Field><Field label="URL" hint="Internal paths must start with /. External links must use https://."><Input value={link.href} maxLength={2048} onChange={(event) => updateLink(link.id, { href: event.target.value })} /></Field><Field label="Category"><Input value={link.category} maxLength={80} onChange={(event) => updateLink(link.id, { category: event.target.value })} /></Field><Field label="Icon"><select value={link.icon} onChange={(event) => updateLink(link.id, { icon: event.target.value })} className="h-9 w-full border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-primary">{iconOptions.map((icon) => <option key={icon} value={icon}>{icon}</option>)}</select></Field></div>
