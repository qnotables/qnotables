...
import { PRODUCTS, dollars } from "@/lib/products"
...
export function MerchGrid() {
  ...
  const [selected, setSelected] = useState<Record<string, string>>(
    Object.fromEntries(
      PRODUCTS.filter((p) => p.type === "good").map((p) => [p.id, productUI[p.id]?.options[0] || ""]),
    ),
  )
    ...
    <>
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {goods.map((product) => {
          ...
          if (!ui) return null
          return (
            <section key={product.id} className="corner-frame flex flex-col border border-border bg-card">
              ...
              <div className="flex flex-1 flex-col p-4">
                ...
                <div className="mt-4">
                  <span className="label-mono text-muted-foreground">{ui.optionLabel}</span>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {ui.options.map((opt) => (
                      <button
                        key={opt}
                        type="button"
                        onClick={() => setSelected((prev) => ({ ...prev, [product.id]: opt }))}
      ...
      <CheckoutDialog request={request} onOpenChange={(open) => !open && setRequest(null)} />
    </>
  )
}
